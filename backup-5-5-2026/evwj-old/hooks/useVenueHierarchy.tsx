...
interface VenueHierarchyContextType {
	...
	addSection: (
		locationId: string,
		section: Omit<VenueSection, "id" | "arrangements">,
	) => void;
	addArrangement: (
		sectionId: string,
		arrangement: Omit<SeatingArrangement, "id">,
	) => void;
	updateArrangement: (
		arrangementId: string,
		updates: Partial<SeatingArrangement>,
	) => void;
	updateSection: (
		sectionId: string,
		updates: Partial<VenueSection>,
	) => void;
...
const VenueHierarchyContext = createContext<
	VenueHierarchyContextType | undefined
>(undefined);
...
export const useVenueHierarchy = () => {
	...
	if (!context) {
		throw new Error(
			"useVenueHierarchy must be used within a VenueHierarchyProvider",
		);
	}
	return context;
};
...
export const VenueHierarchyProvider = ({
	children,
	initialHierarchy,
}: VenueHierarchyProviderProps) => {
	...
	const currentLocation =
		hierarchy.locations.find(
			(l) => l.id === hierarchy?.currentPath?.locationId,
		) || null;
	const currentSection =
		currentLocation?.sections.find(
			(s) => s.id === hierarchy?.currentPath?.sectionId,
		) || null;
	const currentArrangement =
		currentSection?.arrangements.find(
			(a) => a.id === hierarchy?.currentPath?.arrangementId,
		) || null;

	const navigateToArrangement = (
		locationId: string,
		sectionId: string,
		arrangementId: string,
	) => {
			...
			locations: prev.locations.map((location) => ({
				...
				sections: location.sections.map((section) =>
					section.id === sectionId
						? {
								...section,
								arrangements: [...section.arrangements, newArrangement],
							}
						: section,
				),
			})),
	...
	const duplicateArrangement = (arrangementId: string, newName: string) => {
		...
		if (sourceArrangement) {
			const duplicatedArrangement: SeatingArrangement = {
				...
				chairs:
					sourceArrangement.chairs?.map((chair) => ({
						...chair,
						id: Date.now() + Math.random(),
					})) || [],
	...
	const deleteSection = (sectionId: string) => {
		setHierarchy((prev) => {
			...
			if (prev?.currentPath?.sectionId === sectionId) {
				const currentLocation = newHierarchy.locations.find(
					(l) => l.id === prev?.currentPath?.locationId,
				);
		...
		setHierarchy((prev) => {
			const newHierarchy = {
				...
				locations: prev.locations.map((location) => ({
					...
					sections: location.sections.map((section) => ({
						...section,
						arrangements: section.arrangements.filter(
			...
			if (prev?.currentPath?.arrangementId === arrangementId) {
				...
				const currentSection = currentLocation?.sections.find(
					(s) => s.id === prev?.currentPath?.sectionId,
				);
