...
import { Client, DocumentType, ClientDocument } from "../../../mockClients";
...
export default function DocumentsTab({ client, selectedEventId, selectedProjectId }: DocumentsTabProps) {
    ...
    // Use client documents and filter by event/project
    const allDocuments = client.documents || [];
    
    const documents = allDocuments.filter((doc) => {
        if (!selectedEventId && !selectedProjectId) return true;
        
        // Filter by specific project
        if (selectedProjectId) {
            return doc.relatedTo === selectedProjectId || doc.relatedTo.includes(selectedProjectId);
        }
        
        // Filter by event (show all projects in that event)
        if (selectedEventId) {
            const event = client.events?.find((e) => e.id === selectedEventId);
            return event?.projects.some((p) => doc.relatedTo === p.id || doc.relatedTo.includes(p.id));
        }
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [searchQuery, typeFilter, uploadedByFilter, statusFilter, sortOrder, selectedEventId, selectedProjectId]);
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">
            All documents exchanged with {client.name}
          </p>
        </div>
            ...
            <Dialog open={isUploadModalOpen} onOpenChange={handleModalClose}>
                <DialogContent className="sm:max-w-[500px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Select a document type and upload a file to share with {client.name}
                        </DialogDescription>
                    </DialogHeader>
