...
export const Stepper = ({ currentStep }: { currentStep: number }) => {
	...
	return (
		// center items so connector line aligns vertically between circles
		<nav className="flex items-center justify-between w-full p-4 font-inter">
			{steps.map((label, index) => {
                ...
                ${isActive || isCompleted ? "text-foreground font-bold" : "text-muted-foreground font-medium"}`}
						...
						{step < steps.length && (
							// connector centered vertically between circles
							<div
								className={`flex-1 h-0.5 self-center mx-2 sm:mx-4 ${
									isCompleted
