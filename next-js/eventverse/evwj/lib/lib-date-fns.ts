...
export function LibDateFNSValidationTodayOrAfterHelper({
  eventDate,
}: {
  ...
  if (!isAfter(eventDateParsed, today) && !isEqual(eventDateParsed, today)) {
    return false;
  }
...
// Event date (After start date)
export function LibDateFNSValidationAfterDateHelper({
  startDate,
  endDate,
}: {
