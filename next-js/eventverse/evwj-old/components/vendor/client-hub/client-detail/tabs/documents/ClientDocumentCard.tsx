...
import { ClientDocument } from "../../../mockClients";
import { formatFileSize, getDocumentIcon, getStatusBadge, getTypeBadge } from "./DocumentHelpers";
        ...
        <div className="flex flex-wrap gap-1 mb-2 sm:mb-3">
          ...
          >
            <span className="truncate inline-block max-w-full">
              {isUploadedByVendor ? "Uploaded by You" : (
                <>
                  <span className="hidden sm:inline">Uploaded by </span>
                  <span className="truncate inline-block">{clientName}</span>
                </>
              )}
            </span>
          </Badge>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4 mb-2">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground mb-2">
                {document.fileType} • {formatFileSize(document.size)} •{" "}
                {isUploadedByVendor ? "Uploaded by You" : `Uploaded by ${clientName}`} •{" "}
