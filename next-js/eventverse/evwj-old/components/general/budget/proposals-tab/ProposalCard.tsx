...
interface ProposalCardProps {
	...
	onArchive?: () => void;
	onViewVendor?: () => void;
	onViewVendorOverview?: () => void;
	onViewVendorCommunications?: () => void;
	onViewVendorProposals?: () => void;
}
...
export default function ProposalCard({
	...
	onViewDetails,
	onViewVendor,
	onViewVendorOverview,
	onViewVendorCommunications,
	onViewVendorProposals,
}: ProposalCardProps) {
	...
	return (
		<Card className="transition-all hover:border-primary/50 !bg-white dark:!bg-[#020617] backdrop-blur-sm [background-color:rgb(255_255_255/0.95)] dark:[background-color:#020617]">
				...
				<div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
					...
					<div className="flex flex-1 gap-4">
						...
						<div className="flex-1 space-y-3">
							...
							<div className="flex flex-col gap-2 md:hidden">
								...
								<div className="flex gap-2">
									{onViewVendorOverview && (
										<Button variant="outline" size="sm" className="flex-1" onClick={onViewVendorOverview}>
											<User className="mr-1 h-3 w-3" />
											Vendor
										</Button>
									)}
									{onViewVendorCommunications && (
										<Button variant="outline" size="sm" className="flex-1" onClick={onViewVendorCommunications}>
											<MessageSquare className="mr-1 h-3 w-3" />
											Chat
										</Button>
									)}
									{onViewVendorProposals && (
										<Button variant="outline" size="sm" className="flex-1" onClick={onViewVendorProposals}>
					...
					<div className="hidden md:flex md:flex-col md:items-end md:gap-3">
						...
						<div className="flex flex-col gap-2">
							...
							<div className="flex gap-2">
								{onViewVendorOverview && (
									<Button variant="outline" size="sm" onClick={onViewVendorOverview}>
										<User className="mr-2 h-4 w-4" />
										View Vendor
									</Button>
								)}
								{onViewVendorCommunications && (
									<Button variant="outline" size="sm" onClick={onViewVendorCommunications}>
										<MessageSquare className="mr-2 h-4 w-4" />
										Chat
									</Button>
								)}
								{onViewVendorProposals && (
									<Button variant="outline" size="sm" onClick={onViewVendorProposals}>
