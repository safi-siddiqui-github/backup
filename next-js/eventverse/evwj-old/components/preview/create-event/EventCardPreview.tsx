...
const convertFormDataToEvent = (formData: EventFormData) => {
	return {
		...
		startDate: formData.eventDates?.startDate
			? new Date(formData.eventDates.startDate)
			: formData.startDate
...
const EventCardPreview = ({ formData, onEditStep }: Props) => {
	...
	const renderMinimalCard = () => {
		...
		return (
			<Card className="group cursor-pointer !bg-white dark:!bg-slate-800/95 backdrop-blur-sm [background-color:white] dark:[background-color:rgb(30_41_59/0.95)] transition-shadow hover:shadow-lg">
					...
					{(formData.additionalDetails?.ageRestrictions?.hasRestrictions ||
						...
						(formData.additionalDetails?.specialGuests &&
							...
							(() => {
								...
								const specialGuests = formData.additionalDetails.specialGuests;
								if (specialGuests.length === 0) return false;
								if ("guests" in specialGuests[0]) {
									// New format: check if any group has guests
									return (
										specialGuests as Array<{ guests: Array<unknown> }>
									).some((group) => group.guests && group.guests.length > 0);
								} else {
									// Old format: just check length
									return true;
								}
							})()) ||
						...
						<div className="space-y-3 border-t border-gray-200 pt-4">
							...
							{formData.additionalDetails?.specialGuests &&
								...
								(() => {
									...
									const specialGuests =
										formData.additionalDetails.specialGuests;
									if (specialGuests.length === 0) return null;

									// Check if it's the new grouped format
									const firstItem = specialGuests[0];
									const isGroupedFormat = firstItem && "guests" in firstItem;

									// Calculate total guest count and get guests array
									let totalGuests = 0;
									let allGuests: Array<{
										id: string;
										name: string;
									...
									if (isGroupedFormat) {
										// New format: iterate through groups and collect all guests
										const groups = specialGuests as unknown as Array<{
											id: string;
											type: string;
											guests: Array<{
												id: string;
										...
										groups.forEach((group) => {
											if (group.guests && group.guests.length > 0) {
												totalGuests += group.guests.length;
												allGuests.push(
										...
										// If no guests in grouped format, don't render
										if (totalGuests === 0) return null;
									} else {
										// Old format: flat array of guests
										const flatGuests = specialGuests as unknown as Array<{
											id: string;
											name: string;
											title: string;
											bio?: string;
										...
										totalGuests = flatGuests.length;
										allGuests = flatGuests;
									}

									// Group guests by type for display (only if grouped format)
									const guestsByGroup = isGroupedFormat
										? (() => {
												const groups = specialGuests as unknown as Array<{
													id: string;
													type: string;
									...
									return (
										...
										>
											...
											<CollapsibleContent className="mt-2">
												<div className="space-y-4 rounded-lg border border-purple-200 bg-white p-3">
													{isGroupedFormat && guestsByGroup
														? // New format: iterate through groups and their guests
