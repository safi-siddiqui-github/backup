...
import NikeButtonTwoComponent, { NikeButtonTwoIconEnum, NikeButtonTwoTypeEnum } from "@/components/nike/buttons/NikeButtonTwoComponent";
import NikeCardFourComponent, { NikeCardFourProps } from "@/components/nike/cards/NikeCardFourComponent";
import { shopData } from "@/lib/nike/constants/data";
import Form from "next/form";
import { useSearchParams } from "next/navigation";
import { ChangeEvent, SyntheticEvent, useCallback, useEffect, useState } from "react";

export type ShopDataTypeExtended = NikeCardFourProps & {
...
export default function NikeShopClientComponent() {
    ...
    if (searchParams.size > 0) {
      ...
      const filtered = shopData.filter(({ id, category, subCategories, color }) => {
        ...
        if (searchParams.has('subcategory')) { // Check for 'subcategory' key
          ...
          subCategoryMatch = subCategories.some(sub => searchParamsSC.includes(sub.toLowerCase()));
        }

        if (colorExist) {
          const searchColors: string[] = searchParams.getAll("color").map(c => c.toLowerCase());
          colorMatch = color.some(c => searchColors.includes(c.toLowerCase()));
        }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col items-start sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <NikeButtonTwoComponent
          title="Filters"
          type={NikeButtonTwoTypeEnum.BUTTON}
          icon={NikeButtonTwoIconEnum.FILTER}
          onClickFN={toggleFilterMenu}
        />
      </div>
        ...
        <div className={`flex-1 grid grid-cols-1 place-content-start rtd gap-5 ${showFilterMenu ? 'sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3' : 'sm:grid-cols-2 lg:grid-cols-3'}`}>
          {
            shopPageData.map((eachShopDataItem, index) => (
              <NikeCardFourComponent key={`shopData-${index}`} cardFourData={eachShopDataItem} />
            ))
          }
        </div>
