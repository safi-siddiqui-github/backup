...
import { NikeCookieConsentType, useNikeCookieConsentStore } from "@/lib/nike/storage";
import Link from "next/link";
import { useCallback, useEffect, useRef } from "react";

export default function NikeCookieDialogComponent() {
  ...
  const {
    ...
    setCloseDialogForever,
    closeDialogForever,
  }: NikeCookieConsentType = useNikeCookieConsentStore();
  ...
  useEffect(() => {
    if (!closeDialogForever) {
      openModal();
    } else {
      closeModal();
    }
  }, [closeDialogForever, openModal, closeModal]);

  return (
    <dialog ref={dialogRef} className="rounded-md shadow-lg py-6 px-2 flex-col backdrop:backdrop-blur-sm backdrop:bg-transparent md:w-3/4 lg:w-1/2 hidden">
