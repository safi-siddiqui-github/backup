...
export const calculateEventReadiness = (event: any): ReadinessResult => {
  const checks: ReadinessCheck[] = [
    ...
    // Module configuration checks
    {
      ...
      passed: event.selectedModules?.includes('rsvp') && event.moduleConfigurations?.rsvp?.configured,
      weight: 15,
      category: 'important',
    ...
    },
    {
      ...
      passed: event.selectedModules?.includes('schedules') && 
              event.scheduleData?.items && event.scheduleData.items.length > 0,
      weight: 10,
      category: 'important',
