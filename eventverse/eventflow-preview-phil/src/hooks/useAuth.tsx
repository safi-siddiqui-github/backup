...
interface AuthContextType {
  ...
  disconnectSocialProfile: (platform: 'linkedin' | 'instagram' | 'facebook' | 'tiktok') => void;
...
const createMockUser = (): User => ({
  ...
  organizationMemberships: [
    {
      ...
      role: 'owner',
      logoUrl: 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=100&h=100&fit=crop',
      verifiedBadge: true,
  ...
  personalProfile: {
    ...
    interests: ['Travel', 'Photography', 'Cooking', 'Hiking', 'Coffee Culture', 'Wine Tasting'],
    personalLinks: {
    ...
    personalPhoto: 'https://images.unsplash.com/photo-1494790108755-2616b612b786?w=400&h=400&fit=crop&crop=face'
  },
  ...
  professionalProfile: {
    ...
    professionalPhoto: 'https://images.unsplash.com/photo-1494790108755-2616b612b786?w=400&h=400&fit=crop&crop=face',
    ...
    linkedinProfile: {
      ...
      profileData: {
        ...
        summary: 'Award-winning event planner specializing in luxury experiences',
        experience: [
          {
            company: 'Elite Events & Co.',
    ...
    instagram: {
      ...
      profileData: {
        ...
        recentPosts: [
          {
            id: '1',
            imageUrl: 'https://images.unsplash.com/photo-1519225421980-715cb0215aed?w=600&h=600&fit=crop',
  ...
  // Profile metadata
  coverPhoto: 'https://images.unsplash.com/photo-1519671482749-fd09be7ccebf?w=1200&h=400&fit=crop',
  ...
  organizerProfile: {
    ...
    eventTypes: ['Luxury Weddings', 'Corporate Events', 'Tech Conferences', 'Product Launches', 'Charity Galas'],
    serviceAreas: ['San Francisco Bay Area', 'Napa Valley', 'Silicon Valley', 'Monterey Peninsula'],
    priceRange: {
      min: 5000,
      max: 150000,
      currency: 'USD'
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      const enhancedUser = {
        ...createMockUser(),
        ...parsedUser,
        joinDate: parsedUser.joinDate ? new Date(parsedUser.joinDate) : new Date(),
  ...
  const calculateProfileCompleteness = (userData: Partial<User>): number => {
    const fields = [
      'name', 'email', 'phone', 'location', 'bio', 'profilePhoto',
      'company', 'jobTitle', 'industry', 'website'
    ];
    const filledFields = fields.filter(field => userData[field as keyof User]);
  ...
  const updateUser = (updates: Partial<User>) => {
    if (user) {
      const updatedUser = { 
        ...user, 
        ...updates,
        profileCompleteness: calculateProfileCompleteness({ ...user, ...updates })
      };
      setUser(updatedUser);
  ...
  const disconnectSocialProfile = (platform: 'linkedin' | 'instagram' | 'facebook' | 'tiktok') => {
    if (user) {
      const updatedUser = {
        ...user,
        socialIntegrations: {
