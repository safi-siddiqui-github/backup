import { Guest, Table, Seat } from "@/types/venue";
...
export const smartAssignGuests = (
  ...
  const guestGroups = remainingGuests.reduce((groups, guest) => {
    if (!groups[guest.group]) groups[guest.group] = [];
  ...
  for (const [groupName, groupGuests] of Object.entries(guestGroups)) {
    for (const guest of groupGuests) {
      ...
      if (availableTable) {
        const assignedSeats = Object.keys(availableTable.seatAssignments || {}).length;
        if (assignedSeats < availableTable.seats) {
          if (!availableTable.seatAssignments) availableTable.seatAssignments = {};
...
const findBestTable = (
  ...
  const scoredTables = availableTables.map(table => {
    ...
    if (options.prioritizeGroups) {
      const sameGroupGuests = table.guests.filter(g => g.group === guest.group).length;
      score += sameGroupGuests * 50;
    }
    
    // Load balancing - prefer less occupied tables
    if (options.balanceLoad) {
      const occupancy = Object.keys(table.seatAssignments || {}).length / table.seats;
