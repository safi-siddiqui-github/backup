...
import type { Table as TableType, Chair as ChairType, Seat as SeatType, VenueObject as VenueObjectType, LayoutData } from '@/types/venue';
...
interface SeatingCanvasProps {
  ...
  onUpdateVenueObject: (objectId: number, updates: Partial<VenueObjectType>) => void;
...
const SeatingCanvas = forwardRef<any, SeatingCanvasProps>(({
  ...
  const exportImage = useCallback(async () => {
    ...
    try {
      ...
      const controlElements = canvasRef.current.querySelectorAll('[data-ui-control]');
      controlElements.forEach(el => {
        (el as HTMLElement).style.display = 'none';
      });

      // Remove selection highlights
      const selectedElements = canvasRef.current.querySelectorAll('.ring-2, .ring-blue-500, .border-blue-500');
      const originalStyles: { element: HTMLElement; originalClass: string }[] = [];
      selectedElements.forEach(el => {
        const htmlEl = el as HTMLElement;
        originalStyles.push({ element: htmlEl, originalClass: htmlEl.className });
        htmlEl.className = htmlEl.className.replace(/ring-2|ring-blue-500|border-blue-500/g, '');
      });
      ...
      const canvas = await html2canvas(canvasRef.current, {
        ...
        ignoreElements: (element) => {
          return element.hasAttribute('data-ui-control') || 
  ...
  const handleTableSeatAssignment = (table: TableType, seatNumber: number) => {
    onSelectTable(table);
    onAssignSeat();
  };
  ...
  return (
    ...
    >
      <div className="absolute inset-0">
        ...
        {/* Tables */}
        {venueType === "table-based" && tables.map(table => (
          <DraggableTable
            key={table.id}
            table={table}
            ...
            onAssignSeat={(seatNumber) => handleTableSeatAssignment(table, seatNumber)}
            onSwapSeats={() => onSwapSeats(table)}
            viewMode={viewMode}
            draggedGuest={draggedGuest}
            onDropGuestToSeat={onDropGuestToSeat ? (seatNumber) => onDropGuestToSeat(table.id, seatNumber) : undefined}
          />
        ))}

        {/* Individual Chairs */}
        {venueType === "table-based" && chairs.map(chair => (
          <DraggableChair
            key={chair.id}
            chair={chair}
            ...
            onUpdate={(updates) => onUpdateChair(chair.id, updates)}
                ...
                onSelectGrid={() => chair.gridId && onSelectGrid ? onSelectGrid(chair.gridId) : undefined}
                onDelete={() => onDeleteChair(chair.id)}
                onConfigure={() => onConfigureChair(chair)}
                onAssign={() => onAssignChair(chair)}
                onDeleteGrid={chair.gridId && onDeleteChairGrid ? () => onDeleteChairGrid(chair.gridId!) : undefined}
                onUpdateGrid={onUpdateChairGrid}
                gridChairs={chair.gridId && getGridChairs ? getGridChairs(chair.gridId) : []}
                viewMode={viewMode}
              />
            ))}

            {/* Individual Seats */}
            {venueType === "seat-based" && seats.map(seat => (
              <SeatComponent
                key={seat.id}
                seat={seat}
      ...
      {/* UI Controls */}
      {viewMode === "design" && (
        <div data-ui-control className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-lg p-2 shadow-sm">
          <div className="text-xs text-gray-600">
            {tables.length} tables • {chairs.length} chairs • {venueObjects.length} objects
          </div>
        </div>
      )}
    </div>
