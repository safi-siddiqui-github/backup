...
import { Plus, Gift, Mail, QrCode, Copy, Edit, Trash2, UserPlus, Send } from "lucide-react";
...
const TicketReservationSystem = ({ 
  ...
  const handleCreateReservation = (data: any) => {
    const selectedTicketType = ticketTypes.find(t => t.id === data.ticketTypeId);
  ...
  const handleCancelReservation = (reservationId: string) => {
    ...
    const ticketType = ticketTypes.find(t => t.id === reservation.ticketTypeId);
    if (ticketType) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-purple-100">Reserve complimentary tickets for VIPs, staff, and sponsors</p>
        </div>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                          ...
                          <SelectContent>
                            {ticketTypes.filter(t => t.available > 0).map(type => (
      ...
      <div className="space-y-4">
        {reservations.map((reservation) => (
          <Card key={reservation.id} className="bg-white/95 backdrop-blur-sm">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {reservation.ticketTypeName} × {reservation.quantity} • {getReasonLabel(reservation.reason)}
                  </CardDescription>
                </div>
