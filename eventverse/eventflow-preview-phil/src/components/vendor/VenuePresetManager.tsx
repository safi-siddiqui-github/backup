...
import type { VenuePreset, PresetCategory } from "@/types/venue";
...
interface VenuePresetManagerProps {
  vendor: VendorUser;
  venueFloors: any[]; // Floor layouts from enhanced venue management
  onBack: () => void;
}

const VenuePresetManager = ({ vendor, venueFloors, onBack }: VenuePresetManagerProps) => {
  ...
  const presetCategories: PresetCategory[] = [
    { id: "wedding", name: "Wedding", description: "Wedding receptions and ceremonies" },
    { id: "corporate", name: "Corporate", description: "Business events and conferences" },
    { id: "birthday", name: "Birthday", description: "Birthday parties and celebrations" },
    { id: "cocktail", name: "Cocktail", description: "Standing receptions and mixers" },
    { id: "gala", name: "Gala", description: "Formal evening events" },
    { id: "conference", name: "Conference", description: "Professional conferences and seminars" }
  ];
  ...
  const handleCreatePreset = () => {
    const newPreset: VenuePreset = {
      ...
      category: presetCategories.find(c => c.id === formData.category) || presetCategories[0],
    ...
    toast({
      ...
      description: "Your venue preset has been created successfully. You can now design the layout.",
    });
  };
  ...
  const handleEditPreset = () => {
    ...
    const updatedPreset: VenuePreset = {
      ...
      name: formData.name,
      description: formData.description,
      category: presetCategories.find(c => c.id === formData.category) || selectedPreset.category,
      capacity: formData.capacity,
      pricingInfo: {
      ...
      isActive: formData.isActive,
      updatedAt: new Date().toISOString().split('T')[0],
      version: (typeof selectedPreset.version === 'number' ? selectedPreset.version : 1) + 1
    };
  ...
  const openEditDialog = (preset: VenuePreset) => {
    ...
    setFormData({
      ...
      venueType: (preset.venueType === 'seat-based' || preset.venueType === 'table-based') ? preset.venueType : 'table-based',
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => setFormData({ ...formData, pricePerGuest: parseFloat(e.target.value) || 0 })}
            min="0"
            step="0.01"
          />
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{presets.filter(p => p.isActive).length}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Public Presets</p>
                <p className="text-2xl font-bold">{presets.filter(p => p.isPublic).length}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {Math.round(presets.reduce((sum, p) => sum + p.capacity, 0) / presets.length) || 0}
                </p>
              </div>
          ...
          <Card key={preset.id} className="hover:shadow-lg transition-shadow">
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  {preset.isPublic && <Badge variant="secondary" className="text-xs">Public</Badge>}
                  {preset.isActive && <Badge variant="default" className="text-xs">Active</Badge>}
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-1 pt-2">
                <Button size="sm" variant="outline" onClick={() => openEditDialog(preset)}>
                  <Edit3 className="w-3 h-3" />
                </Button>
                <Button size="sm" variant="outline" onClick={() => duplicatePreset(preset)}>
      ...
      {presets.length === 0 && (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">No presets created yet</h3>
            <p className="text-muted-foreground mb-4">
              Create your first venue preset to offer pre-designed layouts to event hosts.
            </p>
            <Button onClick={() => setShowCreateDialog(true)}>
              <Plus className="w-4 h-4 mr-2" />
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Define the basic information for your venue preset. You'll design the layout next.
            </DialogDescription>
          </DialogHeader>
          <FormFields />
          <div className="flex gap-2">
            <Button onClick={handleCreatePreset} disabled={!formData.name || !formData.category}>
              Create Preset
            </Button>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-2xl">
          ...
          <FormFields />
          <div className="flex gap-2">
            <Button onClick={handleEditPreset} disabled={!formData.name || !formData.category}>
              Save Changes
            </Button>
