...
import { Zap, FileText, Calendar, DollarSign, Settings, CheckCircle } from "lucide-react";
...
const InvoiceAutomationPanel = ({ vendor }: InvoiceAutomationPanelProps) => {
  ...
  const recentInvoices = [
    { id: "INV-001", client: "Johnson Wedding", amount: 4250, status: "paid", dueDate: "2024-01-15" },
    { id: "INV-002", client: "Elite Events", amount: 1600, status: "pending", dueDate: "2024-01-20" },
    { id: "INV-003", client: "Birthday Party", amount: 900, status: "overdue", dueDate: "2024-01-10" }
  ];
            ...
            {automationRules.map(rule => (
              <div key={rule.id} className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-gray-600">{rule.trigger} → {rule.action}</p>
                </div>
                <div className="text-right">
                  <Badge variant={rule.status === 'active' ? 'default' : 'secondary'}>
                    {rule.status}
                  </Badge>
                  <p className="text-xs text-gray-500 mt-1">{rule.lastTriggered}</p>
                </div>
              </div>
            ))}
          </div>
