...
const ClientManagementHub = ({ vendor }: ClientManagementHubProps) => {
  ...
  const getStageDistribution = () => {
    const stages: Client["stage"][] = ["prospect", "contacted", "proposal_sent", "negotiating", "contract_signed", "active", "completed"];
    return stages.map(stage => ({
      stage,
      count: clients.filter(c => c.stage === stage).length,
      value: clients.filter(c => c.stage === stage).reduce((sum, c) => sum + c.totalSpent, 0)
    }));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage your client relationships and track their journey</p>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{clients.filter(c => ["prospect", "contacted", "proposal_sent", "negotiating"].includes(c.stage)).length}</p>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="text-gray-500">
                        {Math.round((["prospect", "contacted", "proposal_sent", "negotiating", "contract_signed", "active", "completed"].indexOf(client.stage) + 1) / 7 * 100)}%
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div className="bg-green-50 p-3 rounded">
                      <p className="text-lg font-bold text-green-600">${client.totalSpent.toLocaleString()}</p>
                  ...
                  {client.nextEvent && (
                    <div className="bg-purple-50 p-3 rounded">
                      <p className="font-medium text-purple-800">{client.nextEvent.name}</p>
                      <div className="flex justify-between items-center mt-1">
                        <p className="text-sm text-purple-600">{new Date(client.nextEvent.date).toLocaleDateString()}</p>
                        <p className="text-sm font-medium text-purple-800">${client.nextEvent.value.toLocaleString()}</p>
                      </div>
                    </div>
                  )}
