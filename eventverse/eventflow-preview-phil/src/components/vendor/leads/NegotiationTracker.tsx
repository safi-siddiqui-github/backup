...
const NegotiationTracker = ({ leadId }: NegotiationTrackerProps) => {
  ...
  const negotiationHistory = [
    ...
    },
    {
      ...
      status: "received",
      date: "2024-01-22",
      response: "Client requested to reduce coverage to 6 hours and remove engagement session",
      items: [
  ...
  const negotiationMetrics = {
    ...
    averageAmount: negotiationHistory.reduce((sum, offer) => sum + offer.amount, 0) / negotiationHistory.length,
    priceRange: {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case "initial_proposal": return <FileText className="w-4 h-4" />;
      case "counter_offer": return <TrendingDown className="w-4 h-4" />;
      case "vendor_counter": return <TrendingUp className="w-4 h-4" />;
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{negotiationMetrics.totalOffers}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">${Math.round(negotiationMetrics.averageAmount).toLocaleString()}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{negotiationMetrics.timeInNegotiation}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{negotiationMetrics.closeRate}%</p>
              </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700">Client shows high engagement - 75% probability of closing at $7,500</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700">Suggest offering bonus mini-session to justify $7,500 price point</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-3 bg-yellow-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700">Client typically responds within 24 hours - follow up if no response by tomorrow</p>
              </div>
            </div>
          </div>
        </CardContent>
            ...
            {negotiationHistory.map((item, index) => (
              <div 
                ...
                className={`border rounded-lg p-4 cursor-pointer transition-all ${
                  selectedOffer === item.id ? 'ring-2 ring-blue-500 bg-blue-50' : 'hover:bg-gray-50'
                }`}
                onClick={() => setSelectedOffer(selectedOffer === item.id ? null : item.id)}
              >
