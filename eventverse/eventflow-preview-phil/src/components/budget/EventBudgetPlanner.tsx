...
import type { EventBudgetCategory, BudgetTemplate, SmartSuggestion } from "../../types/budget";
...
const BUDGET_TEMPLATES: BudgetTemplate[] = [
  {
    eventType: "Wedding",
    categories: [
      { name: "Venue", percentage: 40, priority: "high", subcategories: ["Reception Hall", "Ceremony Space", "Setup"] },
      { name: "Catering", percentage: 25, priority: "high", subcategories: ["Food", "Beverages", "Service"] },
      { name: "Photography", percentage: 10, priority: "medium", subcategories: ["Photographer", "Videographer", "Editing"] },
      { name: "Music & Entertainment", percentage: 8, priority: "medium", subcategories: ["DJ", "Band", "Sound System"] },
      { name: "Flowers & Decor", percentage: 8, priority: "medium", subcategories: ["Bouquet", "Centerpieces", "Ceremony Decor"] },
      { name: "Transportation", percentage: 3, priority: "low", subcategories: ["Wedding Car", "Guest Transport"] },
      { name: "Miscellaneous", percentage: 6, priority: "low", subcategories: ["Tips", "Emergency Fund"] }
    ]
  },
  {
    eventType: "Corporate",
    categories: [
      { name: "Venue", percentage: 35, priority: "high", subcategories: ["Conference Room", "AV Equipment", "Catering Space"] },
      { name: "Catering", percentage: 30, priority: "high", subcategories: ["Breakfast", "Lunch", "Coffee Breaks"] },
      { name: "Technology", percentage: 15, priority: "high", subcategories: ["AV Equipment", "Live Streaming", "Tech Support"] },
      { name: "Speakers", percentage: 10, priority: "medium", subcategories: ["Keynote Speaker", "Workshop Leaders"] },
      { name: "Materials", percentage: 5, priority: "medium", subcategories: ["Brochures", "Swag", "Name Tags"] },
      { name: "Marketing", percentage: 5, priority: "low", subcategories: ["Promotion", "Signage", "Photography"] }
    ]
  }
];
...
const EventBudgetPlanner = ({ 
  ...
  const applyTemplate = (template: BudgetTemplate) => {
    const newCategories: EventBudgetCategory[] = template.categories.map((cat, index) => ({
      id: `cat-${index}`,
      name: cat.name,
  ...
  const totalCommitted = categories.reduce((sum, cat) => sum + cat.committed, 0);
  const budgetUtilization = totalBudget > 0 ? (totalPlanned / totalBudget) * 100 : 0;
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {categories.map((category) => {
            const utilization = category.plannedAmount > 0 ? (category.committed / category.plannedAmount) * 100 : 0;
                ...
                <div className="grid grid-cols-3 gap-4 mb-3">
                  ...
                  <div className="text-center">
                    <div className="text-lg font-bold text-gray-600">
                      ${Math.max(0, category.plannedAmount - category.committed).toLocaleString()}
                    </div>
          ...
          {categories.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">Add categories to start planning your event budget</p>
