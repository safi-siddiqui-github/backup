...
const BudgetAnalyticsDashboard = ({
  ...
  const avgProposalValue = proposals.length > 0 ? proposals.reduce((sum, p) => sum + p.totalCost, 0) / proposals.length : 0;
  ...
  const categoryData = expenseItems.reduce((acc, item) => {
    ...
    const categoryProposals = acceptedProposals.filter(p => 
      expenseItems.find(ei => ei.id === p.expenseItemId)?.category === item.category
    );
    acc[item.category].actual += categoryProposals.reduce((sum, p) => sum + p.totalCost, 0);
  ...
  const priorityData = expenseItems.reduce((acc, item) => {
  ...
  const priorityChartData = Object.entries(priorityData).map(([priority, data]) => ({
  ...
  const vendorPerformance = vendors.map(vendor => {
    ...
    const acceptedCount = vendorProposals.filter(p => p.status === 'accepted').length;
    ...
    return {
      ...
      successRate: vendorProposals.length > 0 ? (acceptedCount / vendorProposals.length) * 100 : 0,
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={`text-2xl font-bold ${getVarianceColor(budgetVariance)}`}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-bold">${avgProposalValue.toLocaleString()}</div>
              </div>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={categoryChartData}>
              ...
              <Legend />
              <Bar dataKey="allocated" fill="#8B5CF6" name="Allocated Budget" />
            ...
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  ...
                  fill="#8884d8"
                  dataKey="value"
                  label={({ priority, value }) => `${priority}: $${value.toLocaleString()}`}
                >
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={timelineData}>
                ...
                <Tooltip formatter={(value: number) => [`$${value.toLocaleString()}`, '']} />
                <Legend />
                <Line type="monotone" dataKey="budgeted" stroke="#8B5CF6" name="Budgeted" />
                <Line type="monotone" dataKey="spent" stroke="#10B981" name="Actual Spent" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
            ...
            {vendorPerformance.slice(0, 5).map((vendor, index) => (
              <div key={vendor.name} className="flex items-center justify-between p-4 border rounded-lg">
                ...
                <div className="grid grid-cols-3 gap-6 text-center">
                  <div>
                    <div className="text-lg font-bold text-blue-600">{vendor.proposals}</div>
                    <div className="text-xs text-gray-600">Proposals</div>
                  </div>
                  <div>
                    <div className="text-lg font-bold text-green-600">{vendor.accepted}</div>
                    <div className="text-xs text-gray-600">Accepted</div>
                  </div>
                  <div>
                    <div className="text-lg font-bold text-purple-600">{vendor.successRate.toFixed(0)}%</div>
              ...
              <div className="space-y-2 text-sm">
                {budgetVariance < 0 && (
                  <div className="flex items-start gap-2 p-2 bg-red-100 rounded">
                    ...
                    <span className="text-red-800">
                      You're over budget by ${Math.abs(budgetVariance).toLocaleString()}. Consider renegotiating or finding alternatives.
                    </span>
                  </div>
                )}
                {budgetUtilization < 80 && (
                  <div className="flex items-start gap-2 p-2 bg-green-100 rounded">
                    ...
                    <span className="text-green-800">
                      Great job staying within budget! You have ${budgetVariance.toLocaleString()} remaining.
                    </span>
                  </div>
                )}
                <div className="flex items-start gap-2 p-2 bg-blue-100 rounded">
                  ...
                  <span className="text-blue-800">
                    Consider requesting multiple proposals for remaining items to ensure competitive pricing.
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-start gap-2 p-2 bg-purple-100 rounded">
                  ...
                  <span className="text-purple-800">
                    Focus on vendors with 4.5+ ratings and high response rates for better service.
                  </span>
                </div>
                <div className="flex items-start gap-2 p-2 bg-yellow-100 rounded">
                  <Calendar className="w-4 h-4 text-yellow-600 mt-0.5" />
                  <span className="text-yellow-800">
                    Book high-priority vendors early to secure availability and better rates.
                  </span>
                </div>
              </div>
            </div>
