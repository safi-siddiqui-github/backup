...
const VendorPaymentSchedule = ({
  ...
  const paymentMilestones: PaymentMilestone[] = acceptedProposals.flatMap(proposal => {
  ...
  const paymentsByVendor = paymentMilestones.reduce((acc, payment) => {
  ...
  const totalOverdue = paymentMilestones
    .filter(p => p.status === 'overdue' || (p.status === 'pending' && p.dueDate < new Date()))
    .reduce((sum, p) => sum + p.amount, 0);
  ...
  const getStatusIcon = (payment: PaymentMilestone) => {
    const isOverdue = payment.dueDate < new Date() && payment.status === 'pending';
  ...
  const handlePayment = (payment: PaymentMilestone) => {
    const proposal = acceptedProposals.find(p => p.vendorId === payment.vendorId);
    if (proposal) {
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              ...
              <Progress value={(totalPaid / totalAmount) * 100} className="h-3" />
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              ...
              <div>
                <p className="text-2xl font-bold text-red-600">
                  {totalOverdue > 0 ? Math.round((totalOverdue / totalAmount) * 100) : 0}%
                </p>
                <p className="text-sm text-gray-600">Overdue</p>
              </div>
            </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {Object.entries(paymentsByVendor).map(([vendorId, payments]) => {
                ...
                const vendorPaid = payments.filter(p => p.status === 'paid').reduce((sum, p) => sum + p.amount, 0);
                const vendorTotal = payments.reduce((sum, p) => sum + p.amount, 0);
                ...
                return (
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors ${
                      selectedVendor === vendorId ? 'border-blue-500 bg-blue-50' : ''
                    }`}
          ...
          <CardContent>
            {selectedVendor ? (
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {payment.percentage}% of contract • ${payment.amount.toLocaleString()}
                          </p>
                          <p className="text-sm text-gray-600">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge className={getStatusColor(isOverdue(payment) ? 'overdue' : payment.status)}>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">Select a vendor to view payment details</p>
              </div>
            )}
          </CardContent>
        </Card>
