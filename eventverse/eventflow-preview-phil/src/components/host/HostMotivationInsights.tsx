...
const HostMotivationInsights = ({ 
  ...
  const generateInsights = (): Insight[] => {
    ...
    if (totalEvents < 50) {
      insights.push({
        ...
        description: "Hosts who create 5+ events per month see 3x faster growth. You're on track for amazing success!",
        impact: "medium",
        actionable: true,
        cta: "Get Growth Tips"
      });
  ...
  const analyzeReviewFeedback = (reviews: any[]) => {
    // Simple feedback analysis (in real app, this would use AI)
    const issues = ["communication", "venue setup", "timing", "catering", "entertainment"];
          ...
          <div className="space-y-4">
            {insights.map((insight) => {
              ...
              return (
                <div 
                  key={insight.id}
                  className={`p-4 rounded-lg bg-gradient-to-r ${getInsightColor(insight.type, insight.impact)} text-white`}
                >
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {comparisonStats.map((stat, index) => (
              <div key={index} className="text-center p-4 rounded-lg bg-gradient-to-br from-muted/50 to-muted/30">
                <div className="flex justify-center mb-2">
                  <div className={`p-2 rounded-full ${stat.positive ? 'bg-green-100 dark:bg-green-900/20' : 'bg-gray-100 dark:bg-gray-900/20'}`}>
                    <stat.icon className={`w-5 h-5 ${stat.positive ? 'text-green-600' : 'text-gray-600'}`} />
                  </div>
                </div>
                <div className={`text-xl font-bold ${stat.positive ? 'text-green-600' : 'text-foreground'}`}>
                  {stat.value}
                </div>
              ...
              {recentReviews.slice(0, 3).map((review, index) => (
                <div key={index} className="p-3 rounded-lg bg-muted/30 border border-border">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-muted-foreground">{review.eventName}</span>
                  </div>
