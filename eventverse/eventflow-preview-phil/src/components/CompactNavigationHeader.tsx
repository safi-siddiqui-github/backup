...
import { MapPin, Plus, ChevronDown, Building2, Calendar, Slash } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const CompactNavigationHeader = () => {
  ...
  const [showAddArrangementDialog, setShowAddArrangementDialog] = useState(false);
  ...
  const handleLocationSelect = (locationId: string) => {
    ...
    if (location.sections.length > 0) {
      ...
      if (firstSection.arrangements.length > 0) {
        navigateToArrangement(locationId, firstSection.id, firstSection.arrangements[0].id);
      } else {
  ...
  const handleSectionSelect = (section: any) => {
    ...
    if (section.arrangements.length > 0) {
      navigateToArrangement(selectedLocationId, section.id, section.arrangements[0].id);
    } else {
      // Section exists but no arrangements
  ...
  const handleArrangementSelect = (arrangementId: string) => {
    ...
    const arrangement = currentSection.arrangements.find((a: any) => a.id === arrangementId);
    if (arrangement) {
  ...
  if (hierarchy.locations.length === 0) {
    return (
      <div className="h-10 border-b bg-white px-4 flex items-center">
        <Button size="sm" onClick={() => setShowAddLocationDialog(true)} className="h-7 text-xs">
        ...
        <Dialog open={showAddLocationDialog} onOpenChange={setShowAddLocationDialog}>
          <DialogContent className="sm:max-w-md">
            ...
            <DialogFooter>
              <Button variant="outline" size="sm" onClick={() => setShowAddLocationDialog(false)}>Cancel</Button>
              <Button size="sm" onClick={handleAddLocation} disabled={!newLocationName.trim()}>Add</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
  ...
  return (
    <div className="h-10 border-b bg-white px-4 flex items-center gap-1 text-sm">
      {/* Location Breadcrumb */}
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="sm" className="h-7 px-2 text-xs font-medium text-blue-600 hover:text-blue-700">
      ...
      {/* Separator */}
      {currentLocation && <Slash className="w-3 h-3 text-gray-400" />}
      ...
      {currentLocation && (
        <>
          {currentLocation.sections && currentLocation.sections.length > 0 ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs font-medium text-green-600 hover:text-green-700">
              ...
              <DropdownMenuContent align="start" className="w-48">
                {currentLocation.sections.map(section => (
                  <DropdownMenuItem
                    ...
                    className={currentSection?.id === section.id ? 'bg-green-50' : ''}
                  >
            ...
            <Button size="sm" variant="ghost" onClick={() => setShowAddSectionDialog(true)} className="h-7 px-2 text-xs text-gray-500">
              <Plus className="w-3 h-3 mr-1" />
              Add Section
            </Button>
          )}
      ...
      {/* Separator */}
      {currentSection && <Slash className="w-3 h-3 text-gray-400" />}
      ...
      {currentSection && (
        <>
          {currentSection.arrangements.length > 0 ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs font-medium text-purple-600 hover:text-purple-700">
              ...
              <DropdownMenuContent align="start" className="w-56">
                {currentSection.arrangements.map((arrangement: any) => (
                  <DropdownMenuItem
                    key={arrangement.id}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      {arrangement.startTime && (
                        <div className="text-xs text-gray-500">{arrangement.startTime} - {arrangement.endTime}</div>
                      )}
                    </div>
                  </DropdownMenuItem>
                ))}
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => setShowAddArrangementDialog(true)}>
                  <Plus className="w-3 h-3 mr-2" />
                  Add Arrangement
                </DropdownMenuItem>
              </DropdownMenuContent>
            ...
            <Button size="sm" variant="ghost" onClick={() => setShowAddArrangementDialog(true)} className="h-7 px-2 text-xs text-gray-500">
              <Plus className="w-3 h-3 mr-1" />
              Add Arrangement
            </Button>
          )}
      ...
      <Dialog open={showAddSectionDialog} onOpenChange={setShowAddSectionDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button size="sm" onClick={handleAddSection} disabled={!newSectionName.trim()}>Add</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={showAddArrangementDialog} onOpenChange={setShowAddArrangementDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => setShowAddArrangementDialog(false)}>Cancel</Button>
            <Button size="sm" onClick={handleAddArrangement} disabled={!newArrangementName.trim()}>Add</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
