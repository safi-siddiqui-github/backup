...
const GuestExperienceSimulator = () => {
  const [selectedScenario, setSelectedScenario] = useState("techcon-2024");
  const [selectedModule, setSelectedModule] = useState("games");
  ...
  const mockScenarios = {
    "sarah-michael-wedding": {
      ...
      description: "Join us for an intimate garden wedding celebration surrounded by blooming roses and fairy lights.",
      ticketPrice: 0,
      guestCount: 120,
    ...
    "techcon-2024": {
      ...
      description: "The premier conference for AI professionals, featuring keynotes from industry leaders and hands-on workshops.",
      ticketPrice: 299,
      guestCount: 2500,
    ...
    "company-gala": {
      ...
      description: "Celebrating our team's achievements with dinner, awards, and entertainment.",
      ticketPrice: 75,
      guestCount: 300,
    ...
    "summer-music-fest": {
      ...
      description: "Three days of amazing music, food trucks, and good vibes in the heart of San Francisco.",
      ticketPrice: 89,
      guestCount: 5000,
    ...
    "baby-shower-emma": {
      ...
      description: "Celebrating baby Parker with games, gifts, and sweet treats!",
      ticketPrice: 0,
      guestCount: 35,
    ...
    "milestone-birthday": {
      ...
      description: "Celebrating three decades of awesome with dinner, dancing, and city views!",
      ticketPrice: 45,
      guestCount: 80,
  ...
  const availableModules = selectedEvent.modules.map(moduleId => {
    ...
    return {
      ...
      icon: moduleId === "rsvp" ? Heart : 
            moduleId === "ticketing" ? Ticket :
            moduleId === "schedule" ? Calendar :
            moduleId === "seating" ? Users :
            moduleId === "media" ? Camera :
            moduleId === "games" ? Gamepad2 : MessageSquare
    };
  });
  ...
  const renderModuleContent = () => {
    ...
    switch (selectedModule) {
      ...
      case "seating":
        return <GuestSeatingView guest={selectedGuest} tableNumber={7} seatNumber={3} />;
      case "media":
      ...
      default:
        return <div className="text-center py-8 text-gray-500">Select a module to view the guest experience</div>;
    }
  };
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium mb-2">Event Scenario</label>
                  ...
                  {Object.values(mockScenarios).map((scenario) => (
                    <SelectItem key={scenario.id} value={scenario.id}>
                      <div className="flex items-center gap-2">
                        {scenario.type === "wedding" && <Heart className="w-4 h-4" />}
                        {scenario.type === "conference" && <Building className="w-4 h-4" />}
                        {scenario.type === "corporate" && <Building className="w-4 h-4" />}
                        {scenario.type === "festival" && <Music className="w-4 h-4" />}
                        {scenario.type === "baby-shower" && <Baby className="w-4 h-4" />}
                        {scenario.type === "birthday" && <Gift className="w-4 h-4" />}
                        {scenario.name}
                      </div>
                    </SelectItem>
                  ))}
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Guest Persona</label>
                  ...
                  {Object.entries(guestPersonas).map(([key, persona]) => (
                    <SelectItem key={key} value={key}>
                      <div>
                        ...
                        <div className="text-xs text-gray-500">{persona.relation}</div>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Module View</label>
          ...
          {selectedModule === "games" && (
            <div className="mt-4 grid grid-cols-3 gap-4">
              <div className="text-center p-3 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">{selectedGuest.gameStats.score}</div>
                <div className="text-sm text-gray-600">Guest Score</div>
              </div>
              <div className="text-center p-3 bg-green-50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">#{selectedGuest.gameStats.rank}</div>
                <div className="text-sm text-gray-600">Global Rank</div>
              </div>
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">{selectedGuest.gameStats.gamesPlayed}</div>
