...
const BudgetReports = ({ budgetCategories, expenses, vendors }: BudgetReportsProps) => {
  ...
  const categoryAnalysis = budgetCategories.map(category => {
    ...
    const spent = categoryExpenses.reduce((sum, exp) => sum + (exp.status === "paid" ? exp.amount : 0), 0);
    const committed = categoryExpenses.reduce((sum, exp) => sum + 
      (exp.status === "approved" || exp.status === "paid" ? exp.amount : 0), 0);
    ...
    return {
      ...
      utilization: category.planned > 0 ? (committed / category.planned) * 100 : 0,
  ...
  const vendorAnalysis = vendors.map(vendor => {
    ...
    const totalSpent = vendorExpenses.reduce((sum, exp) => sum + (exp.status === "paid" ? exp.amount : 0), 0);
    const totalCommitted = vendorExpenses.reduce((sum, exp) => sum + 
      (exp.status === "approved" || exp.status === "paid" ? exp.amount : 0), 0);
    ...
    return {
      ...
      averageExpense: vendorExpenses.length > 0 ? totalSpent / vendorExpenses.length : 0
    };
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="text-center p-4 bg-green-50 rounded-lg">
              ...
              <div className="text-2xl font-bold">${(totalBudget - totalCommitted).toLocaleString()}</div>
                ...
                {categoryAnalysis.map((category, index) => (
                  <div key={index} className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge variant={category.utilization > 100 ? "destructive" : "outline"}>
                        {category.utilization.toFixed(0)}%
                      </Badge>
                    </div>
                    <Progress value={Math.min(category.utilization, 100)} className="h-2" />
                    <div className="flex justify-between text-sm text-gray-600">
                      <span>${category.committed.toLocaleString()} / ${category.planned.toLocaleString()}</span>
              ...
              {vendorAnalysis.slice(0, 10).map((vendor, index) => (
                <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">${vendor.totalSpent.toLocaleString()}</div>
