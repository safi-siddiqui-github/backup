...
const LayoutRenderer = ({ layoutData, opacity = 0.3, className = "" }: LayoutRendererProps) => {
  ...
  if (!layoutData.walls?.length && !layoutData.paths?.length && !layoutData.rooms?.length && !layoutData.elements?.length) {
    return null;
  }
