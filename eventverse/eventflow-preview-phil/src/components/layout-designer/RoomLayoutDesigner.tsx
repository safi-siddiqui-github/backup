...
import { 
  Home, Square, DoorOpen, RectangleHorizontal, Circle, Eraser, 
  RotateCcw, Download, Save, Undo, Redo, Ruler, Grid3x3, 
  Move, Copy, RotateCw, Trash2, Settings, Eye, EyeOff 
} from 'lucide-react';
...
interface Room {
  id: string;
  name: string;
  type: 'dining' | 'conference' | 'ballroom' | 'lounge' | 'kitchen' | 'bathroom' | 'other';
  walls: string[];
  area: number;
  color: string;
  capacity?: number;
...
const RoomLayoutDesigner = ({ 
  ...
  const [currentTool, setCurrentTool] = useState<'select' | 'wall' | 'door' | 'window' | 'room' | 'measure' | 'erase'>('wall');
  const [wallType, setWallType] = useState<'exterior' | 'interior' | 'glass'>('interior');
  ...
  const drawElement = (ctx: CanvasRenderingContext2D, element: ArchitecturalElement) => {
    ctx.save();
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (currentTool === 'wall') {
      ...
      const newElement: ArchitecturalElement = {
        id: `door-${Date.now()}`,
        type: 'door',
        x: x - 20,
        y: y - 5,
    ...
    if (e.key === 'Escape') {
      if (isDrawing && currentWall.length > 1) {
        ...
        const newWall: Wall = {
          ...
          color: wallType === 'exterior' ? 'hsl(var(--foreground))' : 
                 wallType === 'glass' ? 'hsl(var(--primary))' : 'hsl(var(--muted-foreground))'
        };
  ...
  }, [isDrawing, currentWall, wallType, rooms.length, scale]);
  ...
  useEffect(() => {
    redrawCanvas();
  }, [walls, rooms, elements, currentWall, showGrid, showMeasurements, showRoomLabels]);
    ...
    <div className="w-full h-full flex flex-col">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input 
              ...
              max="100"
            />
            <span className="text-xs text-muted-foreground">in/px</span>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {onUseAsBackground && (
              <Button size="sm" variant="default" onClick={handleUseAsBackground}>
                Use as Background
              </Button>
            )}
      ...
      <div className="px-4 py-2 bg-muted/50 border-b text-sm text-muted-foreground">
        {currentTool === 'wall' && isDrawing && (
          <span>Click to add wall points • <kbd>Enter</kbd> to close room • <kbd>Esc</kbd> to finish wall</span>
        )}
        {currentTool === 'wall' && !isDrawing && (
          <span>Click to start drawing walls • Hold <kbd>Shift</kbd> for straight lines</span>
        )}
        {currentTool === 'door' && (
        ...
        {currentTool === 'select' && (
          <span>Click to select elements • Drag to move • Right-click for options</span>
        )}
      </div>
      ...
      <div className="flex-1 p-4 overflow-auto bg-muted/30">
        <div className="flex justify-center">
          <canvas
            ...
            width={1200}
            height={800}
            className="border border-border rounded-lg shadow-sm bg-card cursor-crosshair"
      ...
      {/* Status Bar */}
      <div className="h-8 px-4 border-t bg-muted/50 flex items-center justify-between text-xs text-muted-foreground">
        <div className="flex items-center gap-4">
