...
import { Plus, Copy, Edit, Trash2, Users, DollarSign, TrendingUp, Mail } from "lucide-react";
import { useForm } from "react-hook-form";
import { useToast } from "@/hooks/use-toast";
import InfluencerAvatar from "./ticketing/InfluencerAvatar";
...
const ReferralCodeManager = ({ referralCodes, onReferralCodesChange }: ReferralCodeManagerProps) => {
  ...
  const handleCreateReferralCode = (data: any) => {
  ...
  const getConversionRate = (code: ReferralCode) => {
    return code.totalClicks > 0 ? ((code.totalSales / code.totalClicks) * 100).toFixed(1) : '0';
  };

  const getTotalCommissionOwed = () => {
    return referralCodes.reduce((total, code) => total + code.commissionOwed, 0);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-purple-100">Manage influencer partnerships and track commissions</p>
        </div>
        <div className="flex gap-2">
          <div className="text-right">
            <p className="text-sm text-purple-100">Total Commission Owed</p>
            <p className="text-lg font-bold text-yellow-400">${getTotalCommissionOwed().toFixed(2)}</p>
          </div>
          <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(handleCreateReferralCode)} className="space-y-4">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{getCommissionDisplay(code)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Performance</p>
                  <p className="font-medium">{code.totalClicks} clicks</p>
                  <p className="text-sm text-gray-500">{code.totalSales} sales ({getConversionRate(code)}%)</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">${code.totalCommission.toFixed(2)} total</p>
                  <p className="text-sm text-gray-500">${code.commissionPaid.toFixed(2)} paid</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Amount Owed</p>
                  <p className="font-bold text-yellow-600">${code.commissionOwed.toFixed(2)}</p>
                  <Badge variant={code.paymentStatus === 'paid' ? 'default' : 'secondary'}>
                    {code.paymentStatus}
                  </Badge>
                </div>
              </div>
      ...
      {referralCodes.length === 0 && (
        <Card className="bg-white/95 backdrop-blur-sm">
          <CardContent className="py-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No referral partners yet</h3>
            <p className="text-gray-600 mb-4">
              Add influencers and partners to grow your event reach with commission-based referrals.
            </p>
