...
const BudgetAnalytics = ({ budgetCategories, expenses, vendors }: BudgetAnalyticsProps) => {
  ...
  const spendingTrend = dateRange.map(date => {
    const dayExpenses = expenses.filter(exp => 
      format(exp.date, 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd') && exp.status === "paid"
    );
    return {
  ...
  const categoryBreakdown = budgetCategories.map(category => {
    const categoryExpenses = expenses.filter(exp => exp.category === category.name && exp.status === "paid");
                ...
                <div key={index} className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <div className="text-sm">${category.spent.toLocaleString()} / ${category.planned.toLocaleString()}</div>
                      <Badge variant={category.utilization > 100 ? "destructive" : "outline"} className="text-xs">
            ...
            {vendorPerformance.slice(0, 5).map((vendor, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm text-gray-600">{vendor.expenseCount} expenses</div>
                </div>
              </div>
            ))}
          </div>
