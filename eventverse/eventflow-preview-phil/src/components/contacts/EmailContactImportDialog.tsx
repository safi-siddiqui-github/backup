...
import { Search, Mail, Download, CheckCircle, AlertCircle, Users } from "lucide-react";
...
const EmailContactImportDialog = ({ isOpen, onClose, onImport }: EmailContactImportDialogProps) => {
  const [selectedProvider, setSelectedProvider] = useState<string>('');
  const [isLoading, setIsLoading] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [contacts, setContacts] = useState<EmailContact[]>([]);
  const [selectedContacts, setSelectedContacts] = useState<Set<string>>(new Set());
  const [importStep, setImportStep] = useState<'provider' | 'select' | 'success'>('provider');

  // Mock contacts data
  const mockContacts: EmailContact[] = [
    { id: '1', name: 'John Smith', email: 'john.smith@example.com', organization: 'ABC Corp' },
    { id: '2', name: 'Jane Doe', email: 'jane.doe@company.com', organization: 'XYZ Inc' },
    { id: '3', name: 'Mike Johnson', email: 'mike.j@startup.co', organization: 'Startup Co' },
    { id: '4', name: 'Sarah Wilson', email: 'sarah.wilson@freelance.com' },
    { id: '5', name: 'David Brown', email: 'david.brown@agency.net', organization: 'Creative Agency' },
    { id: '6', name: 'Lisa Chen', email: 'lisa.chen@tech.com', organization: 'Tech Solutions' },
    { id: '7', name: 'Robert Taylor', email: 'robert.t@consulting.biz', organization: 'Business Consulting' },
    { id: '8', name: 'Emma Davis', email: 'emma.davis@design.studio', organization: 'Design Studio' }
  ];
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl max-h-[80vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            {importStep === 'provider' && "Choose your email provider to import contacts"}
          ...
          <div className="space-y-4">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-8 space-y-4">
                ...
                <p className="text-xs text-muted-foreground">Fetching your contacts securely</p>
              </div>
            ) : (
