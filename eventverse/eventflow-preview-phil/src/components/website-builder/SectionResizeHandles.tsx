...
export const SectionResizeHandles = ({
  ...
  const handleMouseDown = useCallback((direction: ResizeDirection, e: React.MouseEvent) => {
    ...
    const sectionElement = e.currentTarget.closest('[data-section-id]') as HTMLElement;
    ...
    if (section.positioning?.type === 'absolute' && containerRef.current) {
      ...
      initialPosition.current = {
        x: sectionRect.left - containerRect.left + containerRef.current.scrollLeft,
        y: sectionRect.top - containerRect.top + containerRef.current.scrollTop
      };
    } else {
  ...
  }, [section, onResize, onResizeStart, onResizeEnd, constrainToBounds, resizeDirection]);
    ...
    <>
      {(Object.keys(handleStyles) as ResizeDirection[]).map((direction) => (
        <div
          key={direction}
          className={`
            resize-handle absolute w-3 h-3 bg-primary rounded-full border-2 border-white
            shadow-md hover:scale-125 transition-all duration-150 z-40 cursor-pointer
            ${handleStyles[direction]}
            ${isResizing && resizeDirection === direction ? 'scale-125 bg-primary-foreground shadow-lg' : ''}
          `}
