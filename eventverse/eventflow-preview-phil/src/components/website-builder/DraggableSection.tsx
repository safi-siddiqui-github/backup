...
export const DraggableSection = ({
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (target.closest('.resize-handle') || target.closest('.action-button')) {
      e.stopPropagation();
      return;
    }
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    const draggedIndex = parseInt(e.dataTransfer.getData('application/x-section-index'));
    
    // Only drop if it's a valid section and not dropping on itself
    if (draggedSectionId && section.id !== draggedSectionId && !isNaN(draggedIndex)) {
      onDrop(index);
    }
  ...
  return (
    <div
      ...
      draggable={false}
      className={`
        relative group transition-all duration-200 min-h-[60px]
        ${!isPreviewMode ? 'hover:ring-2 hover:ring-primary/50 hover:shadow-md' : ''}
        ${isSelected ? 'ring-2 ring-primary shadow-lg' : ''}
        ${isDragOver && draggedIndex !== index ? 'ring-2 ring-blue-500 ring-dashed bg-blue-50/50' : ''}
        ${isDragging ? 'opacity-50 scale-95 shadow-2xl' : ''}
        ${isResizing ? 'pointer-events-none' : ''}
      `}
      style={{
        position: section.positioning?.type === 'absolute' ? 'absolute' : 'relative',
        left: section.positioning?.type === 'absolute' ? section.positioning.coordinates.x : undefined,
        top: section.positioning?.type === 'absolute' ? section.positioning.coordinates.y : undefined,
    ...
    >
      ...
      {isDragOver && draggedIndex !== index && draggedIndex !== null && (
        <div className="absolute -top-2 left-0 right-0 h-1 bg-blue-500 rounded-full z-20 animate-pulse shadow-lg">
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-blue-500 text-white text-xs px-2 py-1 rounded whitespace-nowrap">
            Drop here
          </div>
        </div>
      )}
      ...
      {isDragOver && draggedIndex !== index && draggedIndex !== null && (
        <div className="absolute -bottom-2 left-0 right-0 h-1 bg-blue-500 rounded-full z-20 animate-pulse shadow-lg" />
      )}
      ...
      {!isPreviewMode && (
        <div className="absolute top-2 left-2 z-10 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <Button
            size="sm"
            variant="secondary"
            className="h-6 px-2 action-button drag-handle cursor-move"
            onMouseDown={(e) => {
              e.stopPropagation();
              if (sectionRef.current) {
                sectionRef.current.draggable = true;
      ...
      {!isPreviewMode && (
        <div className="absolute top-2 right-2 z-10 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <Button
            size="sm"
            variant="secondary"
            className="h-6 px-2 text-xs action-button"
            onClick={(e) => {
              e.stopPropagation();
              const rect = e.currentTarget.closest('[data-section-id]')?.getBoundingClientRect();
              if (rect) {
                const viewportWidth = window.innerWidth;
                const viewportHeight = window.innerHeight;
