...
export const FloatingWindow = ({
  ...
  const getWindowStyle = (): React.CSSProperties => {
    const baseStyle = {
      ...
      zIndex,
      transition: windowState !== 'normal' ? 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)' : 'none'
    };
  ...
  return (
    <Card
      ref={windowRef}
      className={`
        floating-window overflow-hidden select-none
        ${isDragging ? 'shadow-2xl opacity-90' : 'shadow-lg hover:shadow-xl'}
        ${className}
    ...
    >
      ...
      <div
        className={`
          window-header flex items-center justify-between px-3 py-2 
          bg-gradient-to-r from-violet-100 via-purple-100 to-blue-100 
          border-b border-violet-200 cursor-move select-none
          ${windowState === 'minimized' ? 'rounded-b-lg' : ''}
        `}
        onMouseDown={handleMouseDown}
      >
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <GripVertical className="w-4 h-4 text-violet-600 flex-shrink-0" />
          {type && (
        ...
        <div className="flex gap-1">
          <Button
            variant="ghost"
            size="sm"
            className="h-7 w-7 p-0 hover:bg-yellow-200/80 text-gray-600"
            onClick={(e) => {
              e.stopPropagation();
              handleMinimize();
            }}
          ...
          <Button
            variant="ghost"
            size="sm"
            className="h-7 w-7 p-0 hover:bg-green-200/80 text-gray-600"
            onClick={(e) => {
              e.stopPropagation();
              handleMaximize();
            }}
          ...
          <Button
            variant="ghost"
            size="sm"
            className="h-7 w-7 p-0 hover:bg-red-200/80 text-gray-600"
            onClick={(e) => {
              e.stopPropagation();
              handleClose();
            }}
        ...
        <div className="window-content relative h-full overflow-hidden">
          ...
          {windowState === 'normal' && (
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, transparent 0%, transparent 30%, #8b5cf6 30%, #8b5cf6 100%)',
