...
const CreateArrangementDialog = ({
  ...
  const [step, setStep] = useState<"location" | "section" | "arrangement">("location");
  ...
  const [venueType, setVenueType] = useState<"table-based" | "seat-based">("table-based");
    ...
    <Dialog open={open} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "location" && "Choose or create a location for your arrangement"}
            {step === "section" && "Choose or create a section within the location"}
        ...
        <div className="space-y-4 py-4">
          ...
          {step === "location" && (
            <>
              <RadioGroup value={locationMode} onValueChange={(v: any) => setLocationMode(v)}>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="existing" id="existing-location" />
                  <Label htmlFor="existing-location">Use existing location</Label>
                </div>
                <div className="flex items-center space-x-2">
          ...
          {step === "section" && (
            <>
              <RadioGroup value={sectionMode} onValueChange={(v: any) => setSectionMode(v)}>
                <div className="flex items-center space-x-2">
          ...
          {step === "arrangement" && (
            <>
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={venueType} onValueChange={(v: any) => setVenueType(v)}>
                  <div className="flex items-center space-x-2">
        ...
        <DialogFooter>
          ...
          {step === "location" && (
            <Button onClick={() => setStep("section")} disabled={!canProceedFromLocation}>
              Next
            </Button>
          )}
          
          {step === "section" && (
            <Button onClick={() => setStep("arrangement")} disabled={!canProceedFromSection}>
              Next
            </Button>
          )}
