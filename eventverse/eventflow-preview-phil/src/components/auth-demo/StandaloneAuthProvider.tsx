...
interface StandaloneAuthContextType {
  ...
  inviteUser: (email: string, role: 'admin' | 'viewer' | 'checkin') => Promise<void>;
  updateUserRole: (userId: string, role: 'admin' | 'viewer' | 'checkin') => Promise<void>;
  switchOrganization: (orgId: string) => void;
  acceptInvite: (inviteId: string) => Promise<void>;
}

const StandaloneAuthContext = createContext<StandaloneAuthContextType | undefined>(undefined);

const STORAGE_KEY = 'demo_auth_data';

export const StandaloneAuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      if (data.currentOrgId) {
        const org = data.organizations.find((o: Organization) => o.id === data.currentOrgId);
        if (org) setOrganization(org);
      }
    }
  }, []);

  const saveToStorage = (userData?: StandaloneUser, orgsData?: Organization[], currentOrgId?: string) => {
    const data = {
      user: userData || user,
  ...
  const createOrganization = async (orgData: Partial<Organization>): Promise<Organization> => {
    ...
    const updatedUser = { ...user, organizationId: newOrg.id, role: 'admin' as const, accountType: 'business' as const };
  ...
  const inviteUser = async (email: string, role: 'admin' | 'viewer' | 'checkin') => {
  ...
  const updateUserRole = async (userId: string, role: 'admin' | 'viewer' | 'checkin') => {
  ...
  return (
    <StandaloneAuthContext.Provider value={{
      user,
      organization,
      organizations,
      login,
