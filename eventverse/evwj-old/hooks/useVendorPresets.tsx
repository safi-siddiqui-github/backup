import type { VendorProfile, VenuePreset } from "@/types/venue";
...
export const useVendorPresets = () => {
	...
	const getPresetsByVendor = (vendorId: string) => {
		return vendorPresets.filter(
			(preset) => preset.vendorId === vendorId && preset.isActive,
		);
	};

	const getPresetsByCategory = (categoryId: string) => {
		return vendorPresets.filter(
			(preset) => preset.category.id === categoryId && preset.isActive,
		);
	};
