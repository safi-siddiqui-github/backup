...
const physicalMailContentSchema = z.object({
	...
	bodyContent: z
		...
		.max(2000, "Body content must be 2000 characters or less"),
	callToActionText: z
		.string()
...
export type PhysicalMailContentData = z.infer<typeof physicalMailContentSchema>;

interface PhysicalMailContentFormProps {
	content: PhysicalMailContentData;
	onUpdate: (content: PhysicalMailContentData) => void;
}
...
export default function PhysicalMailContentForm({
	content,
	onUpdate,
}: PhysicalMailContentFormProps) {
	const [errors, setErrors] = useState<
		Partial<Record<keyof PhysicalMailContentData, string>>
	>({});
	const [touched, setTouched] = useState<
		Partial<Record<keyof PhysicalMailContentData, boolean>>
	>({});

	// Validate a single field using Zod
	const validateField = (
		field: keyof PhysicalMailContentData,
		value: any,
	): string | undefined => {
		const fieldSchema = physicalMailContentSchema.shape[field];
		...
		<div className="flex flex-col gap-6 pt-4">
			...
			<div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-6 space-y-6">
				<div className="flex items-center gap-2">
					...
					<h3 className="text-sm font-semibold">Physical Mail Content</h3>
				</div>
				...
				<div className="space-y-2">
					...
					<Input
						id="subjectLine"
						placeholder="Enter mail subject/title"
				...
				<div className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-700 p-6 space-y-4">
					...
					<div className="space-y-2 border-b border-gray-200 dark:border-slate-700 pb-4">
						<p className="text-lg font-bold">
							{content.subjectLine || "Mail subject/title"}
						</p>
					</div>
					...
					<div className="space-y-4">
						<div className="text-sm whitespace-pre-wrap text-gray-700 dark:text-gray-300 min-h-[100px]">
							{content.bodyContent || "Mail body content will appear here..."}
						</div>
