...
import { getAvatarColor, GuestAvatar } from "./GuestAvatar";
...
const DraggableTable = ({
	...
	const handleMouseDown = useCallback(
		...
		[viewMode, onSelect, table.x, table.y],
	);
	...
	const handleMouseMove = useCallback(
		...
		[isDragging, isResizing, dragStart.x, dragStart.y, onUpdate],
	);
	...
	const calculateRectangularChairPositions = useMemo(() => {
		...
		return positions;
	}, [table.seats, table.scale, table.shape]);
	...
	const renderChairs = useMemo(() => {
		...
		if (table.shape === "round") {
			...
			// Use the memoized rectangular logic
		...
		calculateRectangularChairPositions,
		onAssignSeat,
		draggedGuest,
		onDropGuestToSeat,
	]);
		...
		>
			...
			>
				<div className="flex h-full w-full flex-col items-center justify-center p-2">
					...
					{table.targetGroup && (
						<div
							className={`mt-0.5 rounded px-1.5 py-0.5 text-xs ${getGroupBadgeColor(table.targetGroup)}`}
						>
							{table.targetGroup}
						</div>
					)}
