...
const CompactNavigationHeader = () => {
	...
	const selectedLocation = hierarchy.locations.find(
		(l: { id: string | null }) => l.id === selectedLocationId,
	);
	...
	const handleEditSection = (e: React.MouseEvent, section: any) => {
		e.stopPropagation();
	...
	const handleSaveSection = () => {
		if (editingSectionId && editSectionName.trim()) {
			updateSection(editingSectionId, { name: editSectionName.trim() });
	...
	const handleDeleteSection = (e: React.MouseEvent, sectionId: string) => {
		e.stopPropagation();
		if (confirm("Are you sure you want to delete this section? This will also delete all arrangements in this section.")) {
			deleteSection(sectionId);
		}
	};

	const handleEditArrangement = (e: React.MouseEvent, arrangement: any) => {
		e.stopPropagation();
	...
	const handleSaveArrangement = () => {
		if (editingArrangementId && editArrangementName.trim()) {
			updateArrangement(editingArrangementId, { name: editArrangementName.trim() });
			setEditingArrangementId(null);
			setEditArrangementName("");
		}
	};

	const handleDeleteArrangement = (e: React.MouseEvent, arrangementId: string) => {
		e.stopPropagation();
		const arrangement = currentSection?.arrangements.find((a: any) => a.id === arrangementId);
		if (confirm(`Are you sure you want to delete "${arrangement?.name}"?`)) {
	...
	return (
		<div className="flex h-10 items-center gap-1 border-b bg-background px-4 text-sm">
			{/* Location Breadcrumb */}
			<DropdownMenu>
				<DropdownMenuTrigger asChild>
					<Button
						variant="ghost"
						size="sm"
						className="h-7 px-2 text-xs font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300"
					>
				...
				<DropdownMenuContent align="start" className="w-48">
					{hierarchy.locations.map((location) => (
						<DropdownMenuItem
							...
							className={selectedLocationId === location.id ? "bg-blue-50 dark:bg-blue-900/20" : ""}
						>
			...
			{/* Separator */}
			{currentLocation && <Slash className="h-3 w-3 text-muted-foreground" />}
					...
					{currentLocation.sections && currentLocation.sections.length > 0 ? (
						<DropdownMenu>
							...
							<Button
								variant="ghost"
								size="sm"
								className="h-7 px-2 text-xs font-medium text-green-600 hover:text-green-700 dark:text-green-400 dark:hover:text-green-300"
							>
								...
								{currentLocation.sections.map((section) => (
									...
									>
										<DropdownMenuItem
											...
											className={
												currentSection?.id === section.id ? "bg-green-50 dark:bg-green-900/20" : ""
											}
										>
			...
			{/* Separator */}
			{currentSection && <Slash className="h-3 w-3 text-muted-foreground" />}
						...
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<Button
									variant="ghost"
									size="sm"
									className="h-7 px-2 text-xs font-medium text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300"
								>
										...
										>
											...
											<div className="ml-2 flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
												<Button
													...
													size="icon"
													className="h-5 w-5"
													onClick={(e) => handleEditArrangement(e, arrangement)}
												>
													<Edit className="h-3 w-3" />
												</Button>
												<Button
													...
													size="icon"
													className="h-5 w-5 text-destructive hover:text-destructive"
													onClick={(e) => handleDeleteArrangement(e, arrangement.id)}
												>
													<Trash2 className="h-3 w-3" />
												</Button>
											</div>
			...
			{/* Edit Arrangement Dialog */}
			<Dialog open={editingArrangementId !== null} onOpenChange={(open) => !open && setEditingArrangementId(null)}>
				<DialogContent>
					<DialogHeader>
					...
					<div className="space-y-3">
						<div>
							<Label htmlFor="edit-arrangement-name">Name</Label>
							<Input
								id="edit-arrangement-name"
								value={editArrangementName}
								onChange={(e) => setEditArrangementName(e.target.value)}
								placeholder="e.g., Dinner Setup"
								onKeyDown={(e) => {
									if (e.key === "Enter") {
					...
					<DialogFooter>
						<Button
							variant="outline"
							size="sm"
							onClick={() => setEditingArrangementId(null)}
						>
							Cancel
						</Button>
						<Button
							...
							disabled={!editArrangementName.trim()}
						>
							Save
						</Button>
					</DialogFooter>
