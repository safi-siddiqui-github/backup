...
export const useEventStorage = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (storedEvents) {
        ...
        const userEvents = allEvents.filter((event: EventData) => event.userId === user.id);
  ...
  const saveEvent = (eventData: Omit<EventData, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) => {
    if (!user) return null;
  ...
  const updateEvent = (eventId: string, updates: Partial<EventData>) => {
    ...
    localStorage.setItem('eventflow_events', JSON.stringify(updatedAllEvents));
  ...
  const deleteEvent = (eventId: string) => {
    ...
    // Remove the event
    const updatedAllEvents = allEvents.filter((event: EventData) => 
      !(event.id === eventId && event.userId === user.id)
    );
