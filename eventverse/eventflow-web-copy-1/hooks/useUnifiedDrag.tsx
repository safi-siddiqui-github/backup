import { Coordinates, Section, SectionType } from "@/types/website";
...
export const useUnifiedDrag = ({
  ...
  const startSectionDrag = useCallback(
    (
      ...
      const sectionCoords = section.positioning?.coordinates || {
        x: 0,
        y: 0,
        width: 200,
        height: 100,
  ...
  const updateDragPosition = useCallback(
    (moveEvent: MouseEvent | React.MouseEvent) => {
      ...
      if (dragState.dragType === "resize-section") {
        ...
        const originalCoords =
          dragState.originalSection?.positioning?.coordinates;

        if (originalCoords) {
          newPosition = {
            x: Math.max(0, (originalCoords.width ?? 0) + deltaX),
            y: Math.max(0, (originalCoords.height ?? 0) + deltaY),
          };
        } else {
