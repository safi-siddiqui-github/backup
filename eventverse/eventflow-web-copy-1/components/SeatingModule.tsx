...
import type { Chair, Guest, Seat, Table, VenueObject } from "@/types/venue";
...
const SeatingModule = ({ eventId, onBack }: SeatingModuleProps) => {
  ...
  const handleSmartAssign = () => {
    ...
    console.log(
      ...
      tables.map(
        (t) =>
          `${t.name}: ${Object.keys(t.seatAssignments || {}).length}/${t.seats}`,
      ),
    );
  ...
  const handleSelectChair = (chair: Chair | null) => {
    if (chair?.gridId) {
  ...
  const handleAssignGuestToSeat = (
    ...
    setUnassignedGuests((prev) => {
      ...
      console.log(
        "Unassigned guests will be updated. New count:",
        updated.length,
      );
  ...
  const handleSaveChairConfig = (config: ChairConfig) => {
    if (isCreatingChair) {
      ...
      if (config.layoutType === "single") {
        ...
        for (let row = 0; row < (config?.rows ?? 0); row++) {
          for (let col = 0; col < (config?.columns ?? 0); col++) {
            newChairs.push({
              id: Date.now() + row * (config?.columns ?? 0) + col,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Design your venue layout and assign guests
              </p>
            </div>
          </div>
            ...
            onUnassignGuest={(guest, id) => {
              ...
              if (table) {
                ...
                Object.keys(updatedSeatAssignments).forEach((seatNumber) => {
                  const num = Number(seatNumber); // convert string → number
                  if (updatedSeatAssignments[num]?.id === guest.id) {
                    delete updatedSeatAssignments[num];
                  }
                ...
                setChairs((prev) =>
                  prev.map((chair) =>
                    chair.guest?.id === guest.id
                      ? { ...chair, guest: undefined }
                      : chair,
                  ),
                );
              }
      ...
      {showTemplateDialog && (
        <VenueTemplateDialog
          onSelectTemplate={(template) => {
            setTables(
              template?.tables?.map((t, i) => ({ ...t, id: i + 1 })) ?? [],
            );
            setVenueObjects(
              template?.venueObjects?.map((o, i) => ({ ...o, id: i + 1 })) ??
                [],
            );
