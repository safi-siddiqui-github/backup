...
import { Copy, DollarSign, Gift, Percent, Plus } from "lucide-react";
...
const PromoCodeManager = ({
  ...
  interface PromoCodeFormValues {
    code?: string; // optional, can auto-generate
    name: string;
    description: string;
    // type: "percentage" | "fixed" | "bogo" | "free";
    type: string;
    value: string; // input as string, convert → number
    maxUses: string; // input as string, convert → number
    minOrderAmount: string; // input as string, convert → number
    validFrom: string; // ISO/local string
    validUntil: string; // ISO/local string
    isActive?: boolean; // defaulted true
  }

  // const handleCreatePromoCode = (data: PromoCode) => {
  const handleCreatePromoCode: SubmitHandler<PromoCodeFormValues> = (data) => {
    const newPromoCode: PromoCode = {
      id: `PROMO${promoCodes.length + 1}`.padStart(8, "0"),
      code: data.code || generatePromoCode(),
      name: data.name,
      description: data.description,
      type: data.type as PromoCode["type"],
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <DialogContent className="max-w-2xl">
            ...
            <Form {...form}>
              <form
                onSubmit={form.handleSubmit(handleCreatePromoCode)}
                className="space-y-4"
              >
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {code.totalUses} /{" "}
                      {code.maxUses === 999999 ? "∞" : code.maxUses}
                    </p>
                    <p className="text-sm text-gray-500">
                      {code.usesRemaining} remaining
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      ${code.totalDiscount.toFixed(2)} discounted
                    </p>
                  </div>
                  <div>
