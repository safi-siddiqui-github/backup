...
import type { EventBudgetCategory } from "../../types/budget";
...
const EnhancedExpenseTracker = ({
  ...
  const upcomingPayments = expenses
    .filter((exp) => exp.status === "approved" && exp.dueDate > new Date())
    .sort((a, b) => a.dueDate.getTime() - b.dueDate.getTime());

  const overduePayments = expenses.filter(
    (exp) => exp.status !== "paid" && exp.dueDate < new Date(),
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <Card
          className={`border-${totalVariance.amount >= 0 ? "red" : "green"}-200 bg-${totalVariance.amount >= 0 ? "red" : "green"}-50`}
        >
          <CardContent className="p-4 text-center">
            ...
            <div
              className={`text-2xl font-bold text-${totalVariance.amount >= 0 ? "red" : "green"}-800`}
            >
              {totalVariance.amount >= 0 ? "+" : ""}$
              {totalVariance.amount.toLocaleString()}
            </div>
            <div
              className={`text-sm text-${totalVariance.amount >= 0 ? "red" : "green"}-600`}
            >
        ...
        <Card className="border-red-200 bg-red-50">
          <CardContent className="p-4">
            ...
            <div className="mt-1 text-sm text-red-600">
              Total overdue: $
              {overduePayments
                .reduce((sum, exp) => sum + exp.plannedAmount, 0)
                .toLocaleString()}
            </div>
          </CardContent>
        </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {categories.map((category) => {
                ...
                const categoryPlanned = categoryExpenses.reduce(
                  (sum, exp) => sum + exp.plannedAmount,
                  0,
                );
                const categoryActual = categoryExpenses.reduce(
                  (sum, exp) => sum + exp.actualAmount,
                  0,
                );
                const utilization =
                  category.plannedAmount > 0
                    ? (categoryActual / category.plannedAmount) * 100
                    : 0;
                      ...
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-3 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <span className="font-medium">
                              ${expense.actualAmount.toLocaleString()}
                            </span>
                          </div>
                          <div>
                            <span className="text-gray-600">Variance: </span>
                            <span
                              className={`font-medium ${variance.amount >= 0 ? "text-red-600" : "text-green-600"}`}
                            >
                              {variance.amount >= 0 ? "+" : ""}$
                        ...
                        <div className="mt-2 flex items-center gap-4 text-xs text-gray-600">
                          ...
                          {expense.paidDate && (
                            <span>
                              Paid: {expense.paidDate.toLocaleDateString()}
                            </span>
                          )}
