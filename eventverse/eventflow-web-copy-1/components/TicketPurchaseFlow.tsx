...
import { OrderType } from "@/types/rsvp";
import {
  ArrowLeft,
  Clock,
  CreditCard,
...
import { PromoCode } from "./PromoCodeManager";
import { ReferralCode } from "./ReferralCodeManager";
...
interface TicketPurchaseFlowProps {
  ...
  onBack: () => void;
  onComplete: (orderData: OrderType) => void;
}
...
const TicketPurchaseFlow = ({
  ...
  const [appliedPromoCode, setAppliedPromoCode] =
    useState<Partial<PromoCode> | null>(null);
  const [appliedReferralCode, setAppliedReferralCode] =
    useState<Partial<ReferralCode> | null>(null);
  const { toast } = useToast();

  const subtotal = ticketType.price * quantity;
  const fees = Math.round(subtotal * 0.03); // 3% processing fee
  const promoDiscount = appliedPromoCode
    ? Math.round(subtotal * ((appliedPromoCode?.value ?? 0) / 100))
    : 0;
  ...
  const handlePromoCode = () => {
    ...
    const mockPromoCodes: Partial<PromoCode>[] = [
  ...
  const handleReferralCode = () => {
    ...
    const foundReferral = mockReferralCodes.find(
      (r) => r.code.toUpperCase() === referralCode.toUpperCase() && r.isActive,
    );
  ...
  const handlePayment = async () => {
    ...
    onComplete({
      ...
      referralCode: appliedReferralCode?.code ?? "",
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-purple-100">{ticketType.name}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto max-w-4xl px-4 py-6">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            {currentStep === 2 && (
              <Card className="bg-white/95 backdrop-blur-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    We&apos;ll use this to send you your tickets
                  </CardDescription>
                </CardHeader>
          ...
          <div className="lg:col-span-1">
            <Card className="sticky top-6 bg-white/95 backdrop-blur-sm">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-2 text-xs text-gray-600">
                  ...
                  <p>• Questions? Contact support@eventflow.com</p>
                </div>
              </CardContent>
            </Card>
          </div>
