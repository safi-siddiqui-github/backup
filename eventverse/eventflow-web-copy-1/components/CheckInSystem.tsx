...
const CheckInSystem = ({
  ...
  const checkedInOrders = paidOrders.filter(
    (order) => order.checkInStatus === "checked_in",
  );
  const checkInRate =
    paidOrders.length > 0
      ? Math.round((checkedInOrders.length / paidOrders.length) * 100)
      : 0;
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-col gap-4 md:flex-row">
              ...
              <div className="flex-1">
                ...
                >
                  ...
                  <div className="absolute inset-0 rounded-lg border-2 border-white/30">
                    <div className="absolute top-1/2 left-1/2 h-48 w-48 -translate-x-1/2 -translate-y-1/2 transform rounded-lg border-2 border-blue-500"></div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 space-y-4">
                <div>
                  <h4 className="mb-2 font-medium">Manual Entry</h4>
          ...
          >
            <TabsList className="grid w-full grid-cols-3">
              ...
              <TabsTrigger value="checked-in">
                Checked In ({checkedInOrders.length})
              </TabsTrigger>
              <TabsTrigger value="pending">
                Pending ({paidOrders.length - checkedInOrders.length})
              </TabsTrigger>
            </TabsList>
            ...
            >
              <AttendeeTable
                orders={filteredOrders.filter(
                  (o) => o.checkInStatus === "not_checked_in",
                )}
                onCheckIn={handleCheckIn}
                onCheckOut={handleCheckOut}
              />
            ...
            <TableCell>
              {order.checkInTime ? (
                <span className="text-sm">
                  {order.checkInTime.toLocaleDateString()}{" "}
                  {order.checkInTime.toLocaleTimeString()}
                </span>
              ) : (
