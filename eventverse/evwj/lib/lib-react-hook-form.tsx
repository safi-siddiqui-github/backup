...
import TiptapEditorComponent from "./lib-editor-js";
import { SearchPlacesMapComponent } from "./lib-google-map";
...
>({
  ...
  tiptapUseEditorOptions?: UseEditorOptions;
  ...
  switch (fieldType) {
    ...
    case "radio-group":
      return (
        <FieldSet data-invalid={fieldState?.invalid}>
          ...
          >
            {arrayData?.map((item) => {
              const id = `form-rhf-radio-group-${item?.value}-${name}`;
              return (
                <div
    ...
    case "radio-box":
      return (
        <FieldSet data-invalid={fieldState?.invalid}>
          ...
          >
            {arrayData?.map((item) => {
              const id = `form-rhf-radio-box-${item?.value}-${name}`;
              return (
                <FieldLabel
                  key={id}
                  htmlFor={id}
    ...
    case "checkbox-group":
      return (
        <FieldSet data-invalid={fieldState?.invalid}>
          ...
          >
            {arrayData?.map((item) => {
              const id = `form-rhf-checkbox-group-${item?.value}-${name}`;
    ...
    case "checkbox-box":
      return (
        <FieldSet data-invalid={fieldState?.invalid}>
          ...
          >
            {arrayData?.map((item) => {
              const id = `form-rhf-checkbox-box-${item?.value}-${name}`;
      ...
      return (
        <Field data-invalid={fieldState?.invalid}>
          ...
          >
            ...
            <SelectContent>
              {arrayData?.map((item) => {
                const id = `form-rhf-select-${item?.value}-${name}`;
            ...
            <PopoverContent className="flex flex-col overflow-hidden p-0">
              <Command>
                ...
                <CommandList onWheel={(e) => e.stopPropagation()}>
                  ...
                  <CommandGroup>
                    {arrayData?.map((item) => {
                      const id = `form-rhf-commad-${item?.value}-${name}`;
      ...
      return (
        <Field data-invalid={fieldState?.invalid}>
          ...
          <TiptapEditorComponent
            ...
            value={field.value}
            onChange={(json) => field.onChange(JSON.stringify(json))}
            aria-invalid={fieldState?.invalid}
            options={tiptapUseEditorOptions}
          />
    ...
    case "map-autocomplete":
      return (
        <Field data-invalid={fieldState?.invalid}>
          {label && <FieldLabel htmlFor={name}>{label}</FieldLabel>}

          <div className={cn("flex h-44 flex-col", divProps?.className)}>
            <SearchPlacesMapComponent
              value={field?.value}
              onPlaceChanged={(place) => field.onChange(JSON.stringify(place))}
            />
          </div>
