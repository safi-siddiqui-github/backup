...
type HostMotivationInsightsProps = {
	totalEvents: number;
	totalGuests: number;
	avgRating: number;
	recentReviews: Array<{
...
export default function HostMotivationInsights({
	totalEvents,
	totalGuests,
	avgRating,
	recentReviews,
					...
					<div className="space-y-4">
						{insights.map((insight) => {
							...
							return (
								<div
									key={insight.id}
									className={`rounded-lg bg-linear-to-r p-4 ${getInsightColor(insight.type, insight.impact)} text-white`}
								>
				...
				<CardContent>
					<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
						{comparisonStats.map((stat, index) => (
							<div
								key={index}
								className="from-muted/50 to-muted/30 rounded-lg bg-linear-to-br p-4 text-center"
							>
